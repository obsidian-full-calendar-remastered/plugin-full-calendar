...
- **Task identity recovery:** If an edit payload omits task completion metadata, the provider retains task identity from the cached old event and restores an active `completed: false` state. The patched component preserves unmapped properties and server extensions instead of rebuilding the resource as a generic event.
